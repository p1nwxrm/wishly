...
async def get_followers_connections(db: AsyncSession, target_user_id: int, current_user_id: int) -> List[Dict[str, Any]]:
    """
    Get a list of users who are following the target user, formatted for the UI.
    """
    is_followed_sq = (
...
async def get_following_connections(db: AsyncSession, target_user_id: int, current_user_id: int) -> List[Dict[str, Any]]:
    """
    Get a list of users that the target user is following, formatted for the UI.
    """
    is_followed_sq = (
        select(UserSubscription.subscriber_id)
        .where(
